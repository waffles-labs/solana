...
pub fn spl_token_ids() -> Vec<Pubkey> {
    let temp = solana_sdk::pubkey::Pubkey::new(&spl_token::id().to_bytes());
    let temp_2022 = solana_sdk::pubkey::Pubkey::new(&spl_token_2022::id().to_bytes());
    vec![temp, temp_2022]
}
...
pub fn is_known_spl_token_id(program_id: &Pubkey) -> bool {
    let temp = solana_sdk::pubkey::Pubkey::new(&spl_token::id().to_bytes());
    let temp_2022 = solana_sdk::pubkey::Pubkey::new(&spl_token_2022::id().to_bytes());
    *program_id == temp || *program_id == temp_2022
}
...
pub fn spl_token_native_mint_program_id() -> Pubkey {
    solana_sdk::pubkey::Pubkey::new(&spl_token::id().to_bytes())
}
